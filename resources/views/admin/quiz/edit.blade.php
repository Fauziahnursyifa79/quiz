                ...
                <form action="{{ route('quiz.update', $quiz->id) }}" method="POST">
                        ...
                        <select name="materi_id" class="form-control" required>
                            ...
                            @foreach($materis as $materi)
                                <option value="{{ $materi->id }}"
                                    {{ old('materi_id', $quiz->materi_id) == $materi->id ? 'selected' : '' }}>
                                    {{ $materi->title }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="is_active" class="form-control" required>
                            <option value="1" {{ old('is_active', $quiz->is_active) == '1' ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ old('is_active', $quiz->is_active) == '0' ? 'selected' : '' }}>Nonaktif</option>
                        </select>
                    </div>
